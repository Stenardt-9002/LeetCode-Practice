...
public:
    int maximizeWin(vector<int>& prizePositions, int k) 
    {
        ...
        for(auto c : prizePositions)    
            freq[c]++;
        set<int>position1(prizePositions.begin(),prizePositions.end());
        vector<int> position ;
        for(auto c : position1)
            position.push_back(c);
        int n = position.size();
        int index = 0;
        for (int i = 0; i < n; i++)
        {
            if(position[i]-position[index]>k)
                index++;

            cum_freq[index]+=freq[position[i]];
        }
        multiset<int> temp ;
        for(auto c:cum_freq)
...
int maximizeWin(vector<int>& prizePositions, int k) 
{
    ...
    for (int i = 0; i < n; i++) 
    {
        if (i+k-1>= n) 
            break;
        int j =upper_bound(prizePositions.begin(),prizePositions.end(),prizePositions[i] + k - 1) - prizePositions.begin() - 1;
        int len = min(j - i + 1, k);
        cnt += len;
        i = j;
    }
